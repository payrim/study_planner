...
TODAY_JSON = os.path.join(colors.notes_file, 'todaysdatebook.json')
TRACKER_DB = os.path.join(colors.notes_file, 'trackerbookp.json')
UNACTIVE_DB = os.path.join(colors.notes_file, 'unactivetrackerp.json')
TPRIME_DB = os.path.join(colors.notes_file, 'taskprimep.json')
...
def add_missing_books():
    ...
    if missing_books:
        ...
        for i, book in enumerate(missing_books):
            print(f"{i}. {book['name']} - Pages: {book['pages']}")
        ...
        if choice.lower() != 'q':
            try:
                ...
                if 0 <= index < len(missing_books):
                    ...
                    new_tracker = {
                        "name": book["name"],
                        "amount": book["pages"],  # You mentioned mapping "pages" to "amount"
                        "types": "pages",
...
def main():
    while True:
        load_trackers()
        load_tasks()
        print(f"\n{colors.YELLOW}||Book Planner||{colors.RESET}\n\n{colors.GREEN}Menu:{colors.RESET}")
